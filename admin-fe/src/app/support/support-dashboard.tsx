...
export function SupportDashboard() {
  ...
  useEffect(() => {
    if (selectedItinerary) {
      const generatedRequests: BookingRequest[] = selectedItinerary.activities
        .filter((activity) =>
          ["hotel", "restaurant", "transport"].includes(activity.type)
        )
        .map((activity, index) => ({
          id: `BR${selectedItinerary.id.slice(2)}${String(index + 1).padStart(
            3,
            "0"
          )}`,
        ...
        requests.map((r) =>
          r.id === selectedRequest.id
            ? {
                ...r,
                status: "confirmed",
                notes: `${bookingDetails} - Ref: ${bookingReference}`,
              }
            : r
        )
      );
  ...
  const handleSubmitChange = () => {
    if (selectedRequest) {
      ...
      showToast(
        "Changes Applied",
        "Changes have been applied and the booking is set to pending."
      );
  ...
  const handleSendConfirmation = () => {
    if (selectedItinerary) {
      ...
      showToast(
        "Confirmation Sent",
        `Final itinerary sent to ${selectedItinerary.userName}.`
      );
    }
  };
  ...
  const handleProcessChanges = () => {
    if (selectedItinerary) {
        ...
        requests.map((r, index) =>
          index % 3 === 0
            ? {
                ...
                changeNotes: "User requested alternative option",
              }
            : r
        )
      );
      showToast(
        "Processing Changes",
        "User change requests are being processed."
      );
    }
  };
  ...
  const changeRequests = requests.filter(
    (r) => r.status === "change_requested"
  );
                  ...
                  <CardHeader className="flex flex-row items-center justify-between">
                    <div>
                      <CardTitle className="text-lg">
                        {itinerary.userName}'s Trip to{" "}
                        {itinerary.activities[0].location}
                      </CardTitle>
                      <p className="text-sm text-muted-foreground">
                        Days: {itinerary.days} | Created:{" "}
                        {new Date(itinerary.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                    <Badge
                      variant={
                        itinerary.status === "assigned"
                          ? "default"
            ...
            <Card>
              <CardHeader>
                <div className="flex justify-between items-center">
                  <CardTitle>
                    Itinerary Details - {selectedItinerary.userName}
                  </CardTitle>
                ...
                <p className="text-sm text-muted-foreground">
                  Total days: {selectedItinerary.days} | Status:{" "}
            ...
            <Tabs defaultValue="pending" className="space-y-4">
              <TabsList>
                ...
                <TabsTrigger value="changes">
                  Change Requests{" "}
                  <Badge className="ml-2">{changeRequests.length}</Badge>
                </TabsTrigger>
              </TabsList>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="sm:max-w-[525px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Enter the booking details and reference number to confirm this
                request.
              </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={!bookingDetails || !bookingReference}
              >
                Confirm Booking
              </Button>
            </DialogFooter>
