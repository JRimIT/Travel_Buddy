...
class ApiClient {
  ...
  async getPendingTrips(signal?: AbortSignal) {
    console.log("TEST:", signal);
    return this.request("/supporter/booking_pending", { signal });
  }

  async getAssignedTrips(signal?: AbortSignal) {
    return this.request("/supporter/assigned", { signal });
  }

  async getTripHistory(signal?: AbortSignal) {
    return this.request("/supporter/history", { signal });
  }
}
