    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={[styles.bookCard, { padding: 16, alignItems: "flex-start", marginBottom: 20 }]}>
        <View style={{flexDirection:"row", alignItems:"center", marginBottom:7}}>
          {s.user && <Image
            source={{ uri: s.user.profileImage || `https://ui-avatars.com/api/?name=${s.user.username?.charAt(0) ?? "U"}` }}
          ...
          <View>
            ...
            <View style={{flexDirection: "row", alignItems:"center"}}>
              ...
              <Text style={{
                ...
                <Text style={{fontWeight:"400",fontStyle: "italic", color:colors.placeholderText, marginLeft:8}}>
        ...
        {s.description ? (
          <>
            <MaterialCommunityIcons name="notebook-outline" size={18} color={colors.textSecondary} style={{marginBottom:2}} />
            <Text style={[styles.caption, {color:colors.textDark}]}>{s.description}</Text>
          </>
        ):null}
        <Text style={[styles.date, {marginTop:7, color:colors.textSecondary, fontSize:13}]}>
      ...
      <View style={[styles.bookCard, {padding: 16, marginBottom: 15, width: "100%"}]}>
        ...
        <View style={{flexDirection:"row", justifyContent:"space-around"}}>
          <View style={{alignItems:"center"}}>
            ...
            <Text style={{color:colors.textPrimary}}>Khách sạn</Text>
            <Text style={{fontWeight:"bold", color:colors.primary}}>{(s.budget?.hotel || 0).toLocaleString()} VNĐ</Text>
          </View>
          ...
          <View style={{alignItems:"center"}}>
            ...
            <Text style={{fontWeight:"bold", color:colors.primary}}>{(s.budget?.fun || 0).toLocaleString()} VNĐ</Text>
          </View>
        </View>
      </View>
      ...
      {s.hotelDefault && s.hotelDefault.name && (
        <View style={[styles.footer, {marginTop:6}]}>
          ...
          <Text style={{marginBottom:3, color:colors.textPrimary}}><Ionicons name="business" size={18} color={colors.textSecondary} /> {s.hotelDefault.name || s.hotelDefault.address_line1}</Text>
          <Text style={{marginBottom:3, color:colors.textSecondary}}>{s.hotelDefault.address_line2 || s.hotelDefault.formatted}</Text>
          {s.hotelDefault.phone && (
            <Text style={{marginBottom:3}}><Ionicons name="call" size={12} color={colors.textPrimary} /> {s.hotelDefault.phone || s.hotelDefault.contact?.phone}</Text>
          )}
          {s.hotelDefault.website && (
            <Text style={{marginBottom:2, color: "#2186fb"}}>{s.hotelDefault.website}</Text>
          )}
        </View>
      )}
      ...
      {s.flightTicket && s.flightTicket.content && (
        <View style={[styles.footer]}>
          ...
          <Text style={{color: colors.textPrimary, marginBottom:3}}>
            ...
            &nbsp;{s.flightTicket.content.outboundLeg?.originAirport?.skyCode || "-"} → {s.flightTicket.content.outboundLeg?.destinationAirport?.skyCode || "-"}
          </Text>
          <Text style={{color:colors.textSecondary, marginBottom:2}}>
            Ngày đi: {s.flightTicket.content.outboundLeg?.localDepartureDateLabel || s.flightTicket.content.outboundLeg?.localDepartureDate || "-"}
          </Text>
          <Text style={{color:colors.primary, fontWeight:"bold", marginBottom:3}}>
            Giá: {typeof s.flightTicket.content.price==="string" ? s.flightTicket.content.price : `${s.flightTicket.content.price} USD`}
          </Text>
          <Text style={{color:colors.textSecondary}}>
            {s.flightTicket.content.direct ? "[translate:Bay thẳng]" : "[translate:Có điểm dừng]"}
          </Text>
        </View>
      )}
          ...
          <View key={idx} style={{marginBottom:15, width:"100%"}}>
            ...
            {Array.isArray(day.activities) && day.activities.length > 0 ? (
              day.activities.map((act, i) => (
                <View key={i} style={{
                  flexDirection:"row", alignItems:"flex-start", marginTop:5,
                  backgroundColor:colors.inputBackground, borderRadius:8, padding:8, marginBottom:4
                }}>
                  <MaterialCommunityIcons name="clock-outline" size={16} color={colors.primary} style={{marginTop:1, marginRight:3}} />
                  <Text style={{color:colors.textPrimary, fontSize:15, fontWeight:"bold"}}>{act.time} </Text>
                  <Text style={{marginLeft:6, color: colors.textDark, flex:1}}>{act.name}</Text>
                  {act.cost && (
                    <Text style={{marginLeft:7, color:colors.primary, fontWeight:"bold"}}>+{act.cost.toLocaleString()}đ</Text>
                  )}
                </View>
              ))
            ) : (
            ...
            {day.move && (
              <Text style={{marginLeft:18, color:"#3f486f", fontSize:13, marginTop:1}}>
                <Ionicons name="car-outline" size={13} color="#5374e7"/>
                [translate:Đã di chuyển]: {day.move.label || ""}
              </Text>
            )}
          </View>
        ))}
