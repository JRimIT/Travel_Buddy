...
const mainTransports = [
  {
    key: "plane",
    icon: <MaterialCommunityIcons name="airplane" size={21} color="#41b1ff" />,
    label: "Máy bay",
  },
  {
    key: "train",
    icon: <FontAwesome5 name="train" size={20} color="#7c5efa" />,
    label: "Tàu hỏa",
  },
  {
    key: "bus",
    icon: <Ionicons name="bus" size={22} color="#ed881a" />,
    label: "Xe khách",
  },
];
const vehicles = [
  {
    key: "bike",
    icon: <Ionicons name="bicycle-outline" size={26} color="#33bc7f" />,
    label: "Xe máy",
  },
  {
    key: "car",
    icon: <MaterialCommunityIcons name="car" size={26} color="#3b6bb3" />,
    label: "Ô tô",
  },
  {
    key: "other",
    icon: <FontAwesome name="question" size={25} color="#ea5d4b" />,
    label: "Khác",
  },
];
const transportIcons = {
  "Máy bay": (
    <MaterialCommunityIcons name="airplane" size={24} color="#41b1ff" />
  ),
...
const AISuggestGo = ({ goOptions, recommendGo, warningGo }) => (
  <View>
    <Text style={styles.sectionLabel}>
      Đề xuất phương tiện từ vị trí hiện tại đến nơi đến:
    </Text>
    {goOptions?.map((opt, i) => (
      <Text
        key={i}
        style={{
          color: !opt.possible ? "#ed334d" : "#2196f3",
          fontWeight: !opt.possible ? "bold" : "normal",
          marginBottom: 2,
        }}
      >
        {transportIcons[opt.name] || ""} {opt.name} – Trung bình:{" "}
        {opt.avgCost?.toLocaleString()} VNĐ{" "}
        {opt.possible ? "(đủ tiền)" : "(thiếu tiền)"}
        {opt.reason && (
          <Text style={{ color: "#225545" }}> ({opt.reason})</Text>
        )}
      </Text>
    ))}
    {recommendGo && (
      <Text style={{ color: "#133bb8", fontWeight: "bold", marginTop: 10 }}>
        {transportIcons[recommendGo.name]} Nên đi: {recommendGo.name}.{" "}
        {recommendGo.reason}
      </Text>
    )}
    {warningGo && <Text style={styles.warning}>{warningGo}</Text>}
  </View>
);

const AISuggestInner = ({
  innerMoveOptions,
  recommendInner,
  warningInner,
  specialTips,
  ...
  <View style={{ marginTop: 12 }}>
    <Text style={styles.sectionLabel}>
      Phương tiện đi lại nội thành tại điểm đến:
    </Text>
    {innerMoveOptions?.map((opt, i) => (
      <Text
        key={i}
        style={{
          color: !opt.possible ? "#e8672d" : "#0b9448",
          fontWeight: !opt.possible ? "bold" : "normal",
          marginBottom: 3,
        }}
      >
        {transportIcons[opt.name] || ""} {opt.name}{" "}
        {opt.possible ? "" : "(Ngân sách yếu/ít khả thi)"}{" "}
        {opt.reason && (
          <Text style={{ color: "#935804" }}> ({opt.reason})</Text>
        )}
      </Text>
    ))}
    {recommendInner && (
      <Text style={{ color: "#165b7a", fontWeight: "bold", marginTop: 10 }}>
        {transportIcons[recommendInner.name]} Nên đi lại nội thành bằng:{" "}
        {recommendInner.name}. {recommendInner.reason}
      </Text>
    )}
    {warningInner && <Text style={styles.warning}>{warningInner}</Text>}
...
const AITimeline = ({ segments, warning, tips }) => (
  <View>
    ...
    {segments?.map((seg, idx) => (
      <View style={styles.segmentCard} key={idx}>
        ...
        <Text style={styles.segTransport}>
          {transportIcons[seg.suggestedTransport] || ""}{" "}
          <Text style={{ color: "#238cf1", fontWeight: "bold" }}>
            {seg.suggestedTransport}
          </Text>
          {typeof seg.cost === "number" ? (
            <Text style={{ color: "#176c37" }}>
              {" "}
              • {seg.cost.toLocaleString()} VNĐ
            </Text>
          ) : (
            ""
          )}
    ...
    {warning && <Text style={styles.warningAI}>{warning}</Text>}
    {tips && tips.length ? (
      <View style={styles.tipsAI}>
        <Text style={styles.tipsLabel}>Đặc biệt nên thử:</Text>
        {tips.map((tip, i) => (
...
const TravelTransportScreen = () => {
  ...
  const userProvince = useSelector((state:any) => state.inforUserTravel.userProvince);
  ...
  useEffect(() => {
    setSelectedMainTransport("");
    setAIAutoSuggest(null);
  ...
  if (selectedMainTransport && aiAutoSuggest?.goOptions) {
    const label = mainTransports.find(
      (v) => v.key === selectedMainTransport
    )?.label;
  ...
  const handleSave = () => {
    dispatch(
      setUserFlightBudget(flightBudget ? formatMoney(flightBudget) : "0")
    );
    dispatch(
      setUserTransportType(
        selectedVehicle === "other" ? customVehicle : selectedVehicle
      )
    );
  ...
  useEffect(() => {
    setIsSaved(false);
    setAiSuggestion(null);
  }, [selectedMainTransport, selectedVehicle, customVehicle]);
    ...
    <ScrollView style={styles.scroll} keyboardShouldPersistTaps="handled">
      <View style={styles.container}>
        ...
        <View style={styles.provinceBox}>
          <Ionicons name="location" size={18} color="#2586eb" style={{marginRight: 8}} />
          <Text style={styles.provinceText}>{destination}</Text>
        </View>
        ...
        {aiAutoSuggest && (
          <View style={styles.resultBox}>
            <AISuggestGo
              goOptions={aiAutoSuggest.goOptions}
              recommendGo={aiAutoSuggest.recommendGo}
              warningGo={aiAutoSuggest.warningGo}
            />
            <AISuggestInner
              innerMoveOptions={aiAutoSuggest.innerMoveOptions}
              recommendInner={aiAutoSuggest.recommendInner}
              warningInner={aiAutoSuggest.warningInner}
              specialTips={aiAutoSuggest.specialTips}
            />
            {aiAutoSuggest.summary && (
              <Text style={[styles.summaryTxt, { marginTop: 10 }]}>
                {aiAutoSuggest.summary}
              </Text>
            )}
          </View>
        ...
        <View style={styles.vehiclesRow}>
          {mainTransports.map((v) => (
            <TouchableOpacity
              key={v.key}
              style={[
                styles.mainVehicleBtn,
                selectedMainTransport === v.key && styles.vehicleBtnActive,
              ]}
        ...
        <Text style={styles.sectionLabel}>
          Chọn phương tiện di chuyển tại nơi đến
        </Text>
        <View style={styles.vehiclesRow}>
          {vehicles.map((v) => (
            <TouchableOpacity
              ...
              onPress={() => {
                setSelectedVehicle(v.key);
                if (v.key !== "other") setCustomVehicle("");
              }}
              activeOpacity={0.85}
            >
              {v.icon}
          ...
          >
            {isLoadingTimeline ? (
              ...
              <Text style={styles.submitText}>
                AI Đề xuất lộ trình thông minh
              </Text>
            )}
          </TouchableOpacity>
        )}
        {aiSuggestion && showAISuggestion && (
          <View style={styles.suggestionBox}>
            ...
            {!!aiSuggestion.summary && (
              <Text style={styles.summaryTxt}>{aiSuggestion.summary}</Text>
            )}
          </View>
        )}
        {aiSuggestion && !showAISuggestion && (
          <TouchableOpacity
            onPress={() => setShowAISuggestion(true)}
            style={styles.reopenAIBtn}
          >
            <Ionicons
            ...
            />
            <Text
              style={{ color: "#1b6da3", marginLeft: 5, fontWeight: "bold" }}
            >
              Xem lại AI gợi ý
            </Text>
          </TouchableOpacity>
